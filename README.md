...
- `src/app/api/SignupApi.hx` - an API, this can be shared between the server, the CLI task runner, and the client (using remoting).
- `src/app/controller/HomeController.hx` - our main controller.  Look out for:
...
- `src/app/model/Registration.hx` - a simple model to show you how to save the database.  Look out for:
...
- `src/Config.hx` and `src/conf/*.json` - a quick, clean way to include configuration in your app.
- `src/Server.hx` - the starting point for our web app.  Look out for:
...
- `src/Tasks.hx` - the starting point for our CLI tool.  Look out for:
...
- `www/view/layout.html` - the default layout for our website, using the `erazor` templating engine. Note the `@viewContent` variable where our main content goes.
- `www/view/home/` - the folder for all our views in `HomeController`.  We guess the names based on the controller name (`HomeController` becomes `home/`) and the action name (`thankyou()` becomes `thankyou.html`).
